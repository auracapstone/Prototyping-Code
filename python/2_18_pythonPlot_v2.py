...
def makeSignalFig():
    ...
    for j in range(len(peakX)):
        plt.plot(time2[peakX[j]], cut_signal[peakX[j]],'bo')
    plt.title(len(peakX)/(time2[-1]/1000)*60)
    plt.show()
            ...
            if(cnt>5):
                ...
                if(t>captureTime*timecount):
                    ...
                    if (timecount > 1):
                        ...
                        W, ffTtime, cut_signal, cut_f_signal, f_signal = ourFFT2(paccX,time)
                        peakX = detect_peaks(cut_signal)
                        print peakX
                        drawnow(makeFig)
                        drawnow(makeSignalFig1)
                        drawnow(makeFftFig)
