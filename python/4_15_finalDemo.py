...
def finalDemo():
    ...
    plt.suptitle('Team Aura Demo', fontsize=20, fontweight='bold')
    ...
    data = [['Peak Method (BPM)',breaths_peaks],
            ['Baseline Method (BPM)',int(round(breaths_baseline))],
            [' ',''],
    ...
    plt.title('Raw Data - Sample Time')
    plt.ylabel('m^2/s')
    plt.xlabel('Time(s)')
    ...
    plt.title('Filtered Data - Sample Time')
    plt.ylabel('m^2/s')
    plt.xlabel('Time(s)')
...
try:
    ...
    while True:
        ...
        if (fig_count == 10):
            ...
            #high_values_indices = clipping_data > maxThresh
            #clipping_data[high_values_indices] = maxThresh

            #filter the data and find peaks/baseline
            W_z, ffTtime_z, cut_signal_z, cut_f_signal_z, f_signal_z = ourFFT(plot_raw,accel_Time) #perform FFT
            cross_detect = detect_cross(cut_signal_z) #detect baseline cross
            peakaccZ = detect_peaks(cut_signal_z,mph = 0) #detect peaks 
            ...
            #find the coughing 
            coughNum,coughMin,coughMax = detect_cough(plot_raw,maxThresh,minThresh)
            coughPlot = coughMin
