...
function RespiratoryRateDetection 

M = csvread('1-22_v4.csv')
Y = (M(:,1));
Y1 = Y(10:end);
[Y,KK,P]=Kalman1D(Y1,0.0001);
X1 = linspace(0,length(Y1),length(Y1));
Y2 = Y(3:end);
X2 = linspace(0,length(Y2),length(Y2));
[peakLoc] = peakfinder(Y2);
numBreaths = length(peakLoc);
